...
```{r br.bulk.csv.demo1, purl=FALSE, eval=TRUE}
...
```{r br.bulk.csv.demo2, purl=FALSE, eval=HAVEBBG, include=HAVEBBG, results='hide'}
con=br.open()
```

```{r br.bulk.csv.demo2alt, purl=FALSE, eval=!HAVEBBG, include=!HAVEBBG}
con=NULL
```
```{r br.bulk.csv.demo3, purl=FALSE}
...
```{r br.bulk.csv.demo5, purl=FALSE}
length(data)
names(data)
...
```{r br.bulk.csv.demo6, purl=FALSE, results='hide'}
data=br.bulk.csv(con, "res/br.tck.csv", use.xts=FALSE) 
```

```{r br.bulk.csv.demo7, purl=FALSE}
length(data)
names(data)
class(data$Financial)
...
```{r br.bulk.csv.demo11, purl=FALSE}
...
```{r br.bulk.csv.demo12, purl=FALSE, results='hide'}
...
```{r br.bulk.desc.exem, purl=FALSE, eval=HAVEBBG, results='hide'}
con=br.open()
...
```{r br.bulk.tiks.exem, purl=FALSE, eval=HAVEBBG}
...
```{r connections.exem, purl=FALSE, eval=HAVEBBG, results='hide'}
con=br.open() # Open the connection and get the token and load some data
br.bulk.tiks(con, c("MSFT US", "AMZN US"), addtype=TRUE)
br.close(con) # Use the token to release the connection
```
