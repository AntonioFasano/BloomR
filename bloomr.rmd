...
##rm(list=ls(all=TRUE))
## Set this to FALSE to work at home
HAVEBBG=FALSE
...
OTS(showonly= list(purl=FALSE, eval=FALSE,    include=TRUE))
OTS(runonly=  list(purl=FALSE, eval=TRUE,     include=FALSE))
OTS(showrun=  list(purl=FALSE, eval=TRUE,     include=TRUE))
OTS(purlme=   list(purl=TRUE,  eval=TRUE,     include=FALSE))
OTS(bbgtrue=  list(purl=FALSE, eval=HAVEBBG,  include=HAVEBBG))
OTS(bbgfalse= list(purl=FALSE, eval=!HAVEBBG, include=!HAVEBBG))
...
```{r br.bulk.csv.demo1, purl=FALSE, opts.label='runonly'}
read.csv("res/tickers.csv")
```

```{r br.bulk.csv.demo1.show, purl=FALSE, opts.label='showonly'}
...
```{r br.bulk.csv.demo2, purl=FALSE, opts.label='bbgtrue', results='hide'}
con=br.open()
```

```{r br.bulk.csv.demo2alt, purl=FALSE, opts.label='bbgfalse'}
con=NULL
```
```{r br.bulk.csv.demo3, purl=FALSE, opts.label='runonly'}
data=br.bulk.csv(con, "res/tickers.csv") 
```

```{r br.bulk.csv.demo3.show, purl=FALSE, opts.label='showonly'}
...
```{r br.bulk.csv.demo4, purl=FALSE}
data
```
...
```{r br.bulk.csv.demo5, purl=FALSE, opts.label='showrun'}
length(data)
names(data)
...
```{r br.bulk.csv.demo6, purl=FALSE, opts.label='runonly', results='hide'}
data=br.bulk.csv(con, "res/tickers.csv", use.xts=FALSE) 
```
```{r br.bulk.csv.demo6.show, purl=FALSE, opts.label='showonly'}
data=br.bulk.csv(con, "tickers.csv", use.xts=FALSE) 
```

```{r br.bulk.csv.demo7, purl=FALSE, opts.label='showrun'}
length(data)
names(data)
class(data$Financial)
...
```{r br.bulk.csv.demo8, purl=FALSE, opts.label='runonly',  results='hide'}
data=br.bulk.csv(con, "res/tickers.csv", field = "PX_OPEN") 
```

```{r br.bulk.csv.demo8.show, purl=FALSE, opts.label='showonly'}
...
```{r br.bulk.csv.demo9, purl=FALSE, opts.label='runonly'}
data=br.bulk.csv(con, "res/tickers.csv", cols=c(1,3))
```

```{r br.bulk.csv.demo9.show, purl=FALSE, opts.label='showonly'}
...
```{r br.bulk.csv.demo10, purl=FALSE, opts.label='showrun'}
names(data)
```
...
```{r br.bulk.csv.demo11, purl=FALSE, opts.label='runonly'}
read.csv("res/tickers.eqt.csv")
```

```{r br.bulk.csv.demo11.show, purl=FALSE, opts.label='showonly'}
...
```{r br.bulk.csv.demo12, purl=FALSE, opts.label='runonly', results='hide'}
data=br.bulk.csv(con, "res/tickers.eqt.csv", addtype=TRUE)
```

```{r br.bulk.csv.demo12.show, purl=FALSE, opts.label='showonly'}
data=br.bulk.csv(con, "tickers.eqt.csv", addtype=TRUE)
```

Before going home, don't forget to:

```{r br.bulk.csv.demo13, purl=FALSE}
br.close(con)
```
...
```{r br.bulk.desc.exem, purl=FALSE, opts.label='bbgtrue', results='hide'}
con=br.open()
...
```{r br.bulk.desc.exem.show, purl=FALSE, opts.label='showonly'}
con=br.open()
...
```{r connections.exem, bbgtrue, results='hide'}
con=br.open() # Open the connection and get the token and load some data
br.bulk.tiks(con, c("MSFT US", "AMZN US"), addtype=TRUE)
br.close(con) # Use the token to release the connection
...
require(knitr)
require(rmarkdown)
opts.chunk$set(tidy.opts=list(width.cutoff=60))
knit("bloomr.rmd")
purl("bloomr.rmd")
topics()
