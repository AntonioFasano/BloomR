...
knit2 <- function(this.file, use.bloomr=TRUE) {# Knit this Rmd.
### If use.bloomr=T and we're on Windows, with BloomR shipped Pandoc & LaTeX,

    ## Test we synced with this directory 
    if(!file.exists(this.file)) stop("Can't find", this.file, " to knit. Did you setwd() to 'src/'")    

    ## Test we are in Bloomer, if we say so
    if(use.bloomr && !exists(".br.pathexe"))
        stop("You set 'use.bloomr = TRUE' (which builds this Rmd with BloomR shipped exe's),",
             "\nbut you are not running BloomR.")

    ## Execute expression with modified PATH
    wp <- function(expr)
        if(use.bloomr) .br.pathexe(substitute(expr), quiet = FALSE) else expr
    ...
    knit(this.file)
    topics()    
    wp(render(paste0(fse, '.md'), "html_document"))
    wp(render(paste0(fse, '.md'), pdf_document()))
    wp(render("../README.md", html_document()))
...
setwd("src") # Polymode tends to go to git source otherwise
