...
##    BloomR functions are stored in chunks tagged with the "brfuncs" template and knitted only if step2=F
##    At the end of these chunks functions are moved to "bloomr" enviroment.
##
## Second Step
## -----------
...
## For beta/RTM cycle see betaonly chunk and comment below as appropriate
knit.twice("bloomr.beta.Rmd")
#knit.twice("bloomr.Rmd")
...
## For early beta, set RELEASECAND=F
## For RC, set RELEASECAND=T
## For shipping comment `RELEASECAND=T` and ren bloomr.beta.Rmd -> bloomr.Rmd, modify knit.twice accordingly

RELEASECAND=FALSE

## Calling bloomr.R is necessary to run demos (in step 2) requiring functions not yet present here.
if(grepl("^bloomr\\.beta\\.", current_input()) && !RELEASECAND){
    source("bloomr.R") # <- if you knit twice,  assign('bloomr',...) here deletes bloomr env content
    detach(bloomr)     # "attach" chunk is run source() and here, I neutralise the first  
}

```


```{r brspace, purl= ! grepl("^bloomr\\.beta\\.", current_input())  , opts.label='brfuncs'}
## Added only if this file is not *.beta
...
```{r br.hist.csv, opts.label='brfuncs'}
...
```{r deprecated, opts.label='brfuncs'}
...
## Generate a new Rmd where each occurrence of a special tag string is replaced by the arguments
##    and the specially tagged comments related to the R function following the special string.
## The special tag string is: "HQ @args", I use "HQ" here to mean hash and quote chars and
##    avoid a false detection by parseargs().
## The specially tagged comments are like: "HQ comment...". They can be inline or standalone.
## Standalone tagged comments do not want a blank line following, but:
##    the argument to which it refers, or
##    another tagged comment continuing it (to break long lines), or
##    a standard comment, but not in the middle of tagged comments broken into multiple lines.
## Inner HQ comments should appear on line with a single arguments else only the first arg is parsed
## In "HQ ..." the space following the quote is mandatory and cannot be a tab.
