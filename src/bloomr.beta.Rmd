                 ...
                 weekend=TRUE, holidays=NULL)
{
    ...
    if(use.xts){#nrow(LL[[3]])
        LL = lapply(LL, function(ts) {
            if(nrow(ts)) setNames(xts(as.numeric(ts[,-1]), as.Date(ts[,1])), toupper(field)) else NA
        })
    }
...
.br.raw=function( # Get low level data historical data. For internal use
    ...
    .br.convert.to.type(d$vals, d$types) 
}
...
.br.developer=function(mode=TRUE) { # set debug mode
